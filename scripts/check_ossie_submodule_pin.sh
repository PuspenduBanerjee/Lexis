#!/usr/bin/env bash
# Verify the third_party/OSI submodule points at a commit that actually exists on
...
#   .github/workflows/check-osi-submodule.yml - CI backstop, checked on every push/PR
set -euo pipefail
...
# third_party/OSI ...` below would re-resolve those relative paths against the
# submodule's directory instead - and since third_party/OSI/.git is a gitlink
...
error: $SUBMODULE_DIR is pinned to $PINNED_SHA, which is NOT reachable from any
       remote branch of https://github.com/open-semantic-interchange/OSI.

This usually means someone ran 'git commit' *inside* third_party/OSI (creating a
local-only commit) and then 'git add third_party/OSI' in the superproject picked up
