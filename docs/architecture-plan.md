...
- **dbt-core** (dbt Labs) — investigated one level up from MetricFlow, specifically its native `semantic_models:`/`metrics:` YAML (1.8+, restructured again in the "latest spec" as of 1.12+). That native format is *not* embeddable: parsing raw project YAML requires dbt-core's own `ref()`/Jinja/manifest-resolution machinery, not a standalone library (its former standalone schema package, `dbt-semantic-interfaces`, is now deprecated in favor of an internal MetricFlow package). But there's a much bigger finding: **dbt Core v1.12+ natively parses Ossie JSON documents directly** — drop Ossie-schema JSON into a project's `osi/` directory (or a configured `osi-paths`) and dbt folds it straight into `manifest.json`/`semantic_manifest.json` alongside a dedicated `osi_document.json` artifact, no bespoke dbt-format conversion needed. dbt Labs has stated native dbt YAML and Ossie "coexist for now" while they explore deeper convergence — dbt is moving toward Ossie, not the reverse. **Practical effect on this plan**: the "dbt" integration is nearly free — emit (mostly) our native Ossie JSON to the right location/config rather than hand-writing an Ossie→dbt-YAML converter with its own entity/dimension/measure typing. This is now one of the lowest-effort, highest-leverage targets and should be prioritized alongside Cube.js YAML.
...
```
Ossie YAML (author's model)
    |
    v
[apache-ossie: parse + validate]  <-- depend on/vendor the official package, don't reimplement
    |
    v
[Resolved Model layer]  -- our code: index datasets/fields/metrics,
    ...
    |       -- hand-written, per Ossie converters/README.md's documented field/metric/
    |          relationship mapping tables; dialect selection with ANSI_SQL fallback
    |
    +--> dbt Ossie-document emitter  -- near-free: dbt-core 1.12+ ingests raw Ossie JSON
    |       natively (drop into project's osi/ dir / osi-paths config); mostly a
    |       matter of packaging our own Ossie doc correctly + version/schema checks,
    |       not a bespoke Ossie->dbt-YAML converter
    |
...
lexis/
  ...
  tests/
    fixtures/
      ...
      flights.yaml               # Ossie's ontology-layer example, for later ontology-mapping support
    test_parser.py
...
**Reuse points already identified from Ossie's own repo** (`apache/ossie` on GitHub, formerly `open-semantic-interchange/OSI`):
- `python/src/ossie/models.py` — pydantic `OssieDocument`/`OssieSemanticModel`/`OssieDataset`/`OssieField`/`OssieRelationship`/`OssieMetric` classes with `to_ossie_yaml()`/`to_ossie_json()`. Depend on this directly (check PyPI availability for `apache-ossie`; vendor a pinned copy if not yet published).
- `core-spec/ossie-schema.json` and `validation/validate.py` — for pre-transpile validation.
- `converters/README.md` — the field-by-field mapping table (datasets/fields/relationships/metrics → vendor equivalents, dialect-fallback rules, composite-key handling, custom_extensions round-tripping) is essentially a spec for our SQL emitters; follow it directly rather than re-deriving mapping rules.
...
- dbt Labs' `docs.getdbt.com/docs/build/ossie-semantic-models` (Ossie ingestion in dbt-core 1.12+) — defines the exact `osi/`-directory / `osi-paths` packaging convention and the Ossie schema version dbt currently expects; the `dbt_ossie.py` emitter should target this precisely rather than guessing dbt's expectations.
- (Reference only, not a dependency) dbt's now-deprecated `dbt-semantic-interfaces` validation rules (non-additive dimensions, `agg_time_dimension`, primary/foreign entity join inference) — worth skimming for edge cases Ossie's spec may not yet address, since it's a mature, battle-tested rule set.

**Update**: the OSI project was donated to the ASF and now lives at `apache/ossie` (formerly `open-semantic-interchange/OSI`), checked out as a git submodule at `third_party/ossie` (still not depended on as `apache-ossie` isn't published to PyPI), so the reuse points above are live files, not remote references — `core-spec/ossie-schema.json` is used directly by `tests/test_ossie_spec_conformance.py` to validate fixtures, and `scripts/sync_ossie_vendor.sh` re-vendors `models.py` from the submodule on demand. See README's "Keeping Ossie in sync".

## Verification

1. Parse `tpcds_semantic_model.yaml` via `apache-ossie` into an `OssieDocument`; assert datasets/relationships/metrics counts match the source file.
...
5. Package the Ossie document per dbt-core 1.12+'s `osi/`-directory convention and confirm it matches the expected schema version/location (a real dbt-project ingestion test is a stretch goal if a throwaway dbt project is available).
...
- Whether to depend on `apache-ossie` from PyPI or vendor a pinned copy (check publish status when implementing).
...
- 27 pytest tests green (parser, resolved-model join-graph resolution, all 5 SQL dialect emitters, Cube YAML, dbt Ossie packaging, MCP manifest).
- Generated DuckDB SQL was executed against real tables and produced correct aggregates.
- `dbt_ossie.py` and `mcp.py` were added beyond the original file list (not just `cube.py`) to cover the dbt-core 1.12+ Ossie ingestion path and the MCP/AI-consumer differentiator.
