...
**Phase 3 — round-trip fidelity tests (both directions)** — ✅ **done**
- [x] **`emit.py` made stash-aware** (a prerequisite this phase surfaced: Phase 1's emitter always flattened every relationship-target dataset into a fresh single-level dimension, even for a document that was itself just parsed from a real multi-level SML hierarchy - so a true SML → Ossie → SML round trip would have silently lost that hierarchy on the very next hop). It now reads `read_stash(semantic_model)` and: (a) re-emits a stashed dimension's full original dict verbatim (`_dump_raw`) instead of re-flattening it, resolved to the right dataset via `_stashed_dimensions_by_dataset`; (b) also emits a stashed dimension that has no relationship pointing at it at all (a degenerate dimension keyed on a fact table's own column - `dimension_targets` alone would never catch it); (c) resolves a relationship's `to.level` to the dimension's actual grain-key level (`_leaf_level_of`), not just the dimension's own name, since a re-emitted multi-level hierarchy's leaf level attribute may differ from a freshly-synthesized single-level one only by convention, not by guarantee.
- [x] **"No silent loss" extended to metrics** (mirroring `test_ossie_metric_no_silent_loss.py`'s pattern exactly, adapted to SML's constraints): a metric with no ANSI_SQL expression, or one that decomposes as neither a simple nor a ratio aggregate, is no longer just warned-and-dropped - it's preserved verbatim under a small `x_lexis_unconverted_metrics` escape hatch on the emitted `model.yml`. This is *not* a documented SML field (SML has no general vendor-extension slot the way Ossie's `custom_extensions` is) - it's this converter's own, clearly-namespaced addition, restored by `parse.py` on the way back (with a stale-stash-name-collision rule: a real metric/metric_calc of the same name now present in the repo wins, and the stale entry is dropped with a warning, mirroring `TestStaleStashNameCollision`).
- [x] `tests/test_sml_roundtrip.py`: extended with `TestSmlToOssieToSml` (5 tests) covering the *other* direction - the hand-authored `sml_repo_dir` fixture parsed then re-emitted, checking the 2-level hierarchy re-emits byte-for-byte, the degenerate time dimension (no relationship) still re-emits, the relationship's `to.level` resolves to the correct leaf, and the arbitrary-MDX metric_calc survives two full hops. One test documents a known, accepted gap: an unsupported `object_type` (e.g. `row_security`) round-trips as *retrievable* stash data but isn't re-materialized as its own standalone SML file (its original file's folder placement was never captured to reconstruct) - out of scope for this phase, noted rather than silently left unclear.
- [x] `tests/_sml_roundtrip_helpers.py`: framework-free `Rnd` protocol + `RandomRnd`, `build_ossie_document(rnd)` (1-2 dimension datasets + 1 fact dataset + 1-2 metrics, each either a simple or ratio aggregate, entirely within the v1 round-trippable subset), and `assert_ossie_roundtrip(document_dict)` (dataset names, relationship pairs, and every metric name + its ANSI_SQL dialect survive Ossie → SML → Ossie).
- [x] `tests/test_sml_roundtrip_properties.py` (Hypothesis-backed, `pytest.importorskip("hypothesis")`) + `tests/test_sml_roundtrip_properties_seeded.py` (30 plain-seeded-`random.Random` cases, runs unconditionally) - mirrors `third_party/ossie/converters/databricks/tests/test_roundtrip_properties.py`'s driver/seeded-fallback split exactly. Caught one real generator bug during verification: Hypothesis's shrinker collapsed the identifier alphabet toward a single repeated value, exhausting `_unique`'s 50-retry budget - fixed with a guaranteed-unique numeric-suffix fallback. Verified passing with `hypothesis` installed (100 examples, then removed again - not a project dependency, the whole point is these tests degrade to the seeded fallback without it).
- [x] Full suite green: 205 passed, 1 skipped (the Hypothesis test, cleanly, since `hypothesis` isn't an installed/declared dependency).
