...
**Phase 2 — SML → Ossie**
- [ ] `src/lexis/sml/parse.py`: `parse_sml_repo(directory) -> OssieDocument`. Two-pass: (a) walk every `*.yml`, discriminate by `object_type`, build a global `unique_name → object` index; (b) resolve every reference (`dataset:`, `dimension:`, `connection_id:`, `to.dimension`+`to.level`, `[Measures].[name]` in MDX) against that index, then build datasets/fields/relationships/metrics.
- [ ] New `lexis import sml` CLI command per Architecture above.
- [ ] New fixture: `tests/fixtures/sml/` — a small hand-authored SML repo (one connection, 2-3 datasets, one 2-level hierarchy + one time dimension, 2-3 simple metrics, one relationship, one `metric_calc` to exercise the MDX pass-through path).
- [ ] Tests: round-trip the Phase 1 emitter's own output back through `parse_sml_repo()` and assert structural equivalence "up to documented normalizations" (mirror `strip_normalized()` from `third_party/ossie/converters/omni/tests/_util.py`).
